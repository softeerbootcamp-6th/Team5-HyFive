...
describe("지도의 중점을 올바르게 반환한다.", () => {
  it("전체 경로의 길이가 홀수인 경우 중앙값에 해당하는 위치를 반환한다.", () => {
    //when
    const centerLatLng = getRouteMidPoint(path);
  ...
  it("전체 경로의 길이가 짝수인 경우 중앙 기준 좌측에 해당하는 위치를 반환한다.", () => {
    //when
    const centerLatLng = getRouteMidPoint(path.slice(0, path.length - 1));
  ...
  it("전체 경로의 길이가 1인 경우 해당 요소의 위치를 반환한다.", () => {
    //when
    const centerLatLng = getRouteMidPoint(path.slice(0, 1));
  ...
  it("전체 경로의 길이가 음수거나 0인 경우 null인 위치를 반환한다.", () => {
    //when
    const centerLatLng = getRouteMidPoint(path.slice(0, 0));
